...
class Graph {
    ...
    bfs2(vertex, end) {
        ...
        while (!queue.isEmpty()) {
            ...
            this.edges[head].forEach(connectedVertex => {
                if (!completed[connectedVertex] && !finished) {
                    completed[connectedVertex] = true;
                    findPath.push(connectedVertex)
                    queue.enqueue(connectedVertex)
                }
    ...
    bfsShortest(start, end) {
        this.paths = [];
        const finalPath = [];
        let path = this.bfs2(start, end)
        path = path.slice(0, path.findIndex(x => x === end) + 1)
        const head = path.shift();
        finalPath.push(head);
        if (this.edges[head].includes(end)) {
            return [...finalPath, end]
        }
        let temp = [...path];
        path.forEach(el => {
            const elPath = this.bfs2(el)
            let parent = path.filter(child => this.edges[child].includes(el))
            parent = parent[0] || head;
            if (elPath.includes(end))
                this.paths.push([parent, ...elPath])

            if (elPath.includes(end) && temp.includes(el)) {
                finalPath.push(el);
                temp = [...elPath]
            }

            if (this.edges[el].includes(end)) {
                return [...finalPath, end]
            }
        })
        this.paths = this.paths.filter(x => x.includes(head)).map(x => x.slice(0, x.indexOf(end) + 1)).sort((a, b) => a.length - b.length)
        return this.paths[0]
    }
...
test.only('bfsShortestWay', () => {
    ...
    expect(graph.bfsShortest('A', 'B')).toEqual(['A', 'B']);

    expect(graph.bfsShortest('A', 'D')).toEqual(['A', 'B', 'D']);

    expect(graph.bfsShortest('A', 'C')).toEqual(['A', 'C']);
    ...
    graph.addDirectedEdge('A', 'I');

    graph.addDirectedEdge('I', 'J');

    expect(graph.bfsShortest('A', 'G')).toEqual(['A', 'B', 'D', 'G']);
    ...
    expect(graph.bfsShortest('A', 'F')).toEqual(['A', 'C', 'F']);
